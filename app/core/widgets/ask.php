...
class Knife_Widget_Ask extends WP_Widget {
    /**
     * Widget post types
     */
    private $post_type = ['ask'];
    ...
    public function __construct() {
        $widget_ops = [
            'classname' => 'ask',
            'description' => __('Выводит блок вопросов', 'knife-theme'),
            'customize_selective_refresh' => true
        ];

        parent::__construct('knife_widget_ask', __('[НОЖ] Вопросы', 'knife-theme'), $widget_ops);
    }
    ...
    public function widget($args, $instance) {
        ...
        if($query->have_posts()) {
            ...
            printf('<a class="widget-ask__head head" href="%2$s">%1$s</a>',
                esc_html($instance['title']),
                esc_url(get_post_type_archive_link('ask'))
            );

            while($query->have_posts()) {
                $query->the_post();

                include(get_template_directory() . '/templates/widget-ask.php');
            }
...
add_action('widgets_init', function() {
    register_widget('Knife_Widget_Ask');
});
