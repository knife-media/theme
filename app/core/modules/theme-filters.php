...
class Knife_Theme_Filters {
    ...
    public static function load_module() {
        ...
        add_action( 'init', array( __CLASS__, 'add_pages_excerpts' ) );

        // Remove longreads archive title
        add_filter( 'get_the_archive_title', array( __CLASS__, 'remove_longreads_title' ) );
        ...
        add_filter( 'body_class', array( __CLASS__, 'update_body_classes' ), 10 );

        // Update post classes
        add_filter( 'post_class', array( __CLASS__, 'update_post_classes' ), 10, 2 );

        // Add post author link class
        add_filter( 'the_author_posts_link', array( __CLASS__, 'add_author_class' ), 10, 1 );
        ...
        add_action( 'template_redirect', array( __CLASS__, 'redirect_generator' ), 9 );
        ...
        add_filter(
            'wp_mail_from_name',
            function() {
                return esc_html__( 'knife.media', 'knife-theme' );
            }
        );
    ...
    public static function preload_assets() {
        ...
        foreach ( $fonts as $font ) {
            printf(
                ...
                esc_url( get_template_directory_uri() . '/assets/' . $font )
            );
        }
    }
    ...
    public static function update_roles() {
        ...
        add_role(
            'manager',
            esc_html__( 'Менеджер', 'knife-theme' ),
            array(
                'read'         => true,
                'promo_manage' => true,
                'upload_files' => true,
        ...
        add_filter(
            ...
            function( $result ) {
                ...
                if ( ! is_user_logged_in() ) {
                    return new WP_Error(
                        'rest_not_logged_in',
                        esc_html__( 'Authorization required for all API requests.' ),
                        array( 'status' => 403 )
                    );
                }
    ...
    public static function print_styles_checkbox( $template, $post ) {
        ...
        printf(
            ...
            esc_attr( 'sdf' ),
            esc_html__( 'Добавить форму заявки в клуб', 'knife-theme' ),
