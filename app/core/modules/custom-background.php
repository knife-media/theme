...
class Knife_Custom_Background {
     ...
     * @since   1.3
     */
    public static $taxonomies = array( 'special' );
    ...
    public static function load_module() {
        ...
        // Update Customizer
        add_action( 'customize_register', array( __CLASS__, 'update_customizer' ) );

        // Frontend styles
        add_action( 'wp_enqueue_scripts', array( __CLASS__, 'print_background' ), 13 );
    }
    ...
    public static function add_metabox() {
        if ( current_user_can( 'unfiltered_html' ) ) {
            add_meta_box(
                'knife-background-metabox',
                esc_html__( 'Произвольный фон', 'knife-theme' ),
    ...
    public static function add_options_fields() {
        foreach ( self::$taxonomies as $tax ) {
            ...
            add_action( "edited_{$tax}", array( __CLASS__, 'save_options' ) );
        }
    ...
    public static function print_background() {
        ...
        if ( ! empty( $meta['image'] ) ) {
            ...
            if ( in_array( $meta['size'], array( 'auto', 'contain', 'cover' ), true ) ) {
    ...
    public static function enqueue_options_assets( $hook ) {
        ...
        if ( $hook !== 'term.php' || ! in_array( $screen->taxonomy, self::$taxonomies, true ) ) {
            return;
        }
        ...
        wp_enqueue_script( 'knife-background-options', $include . '/scripts/background-options.js', array( 'jquery', 'wp-color-picker' ), $version, true );

        $options = array(
            'choose' => esc_html__( 'Выберите фоновое изображение', 'knife-theme' ),
        );
    ...
    public static function enqueue_metabox_assets( $hook ) {
        ...
        // Insert admin scripts
        wp_enqueue_script( 'knife-background-metabox', $include . '/scripts/background-metabox.js', array( 'jquery', 'wp-color-picker' ), $version, true );
    ...
    public static function save_meta( $post_id ) {
        ...
        $background = array_filter(
            array_map( 'sanitize_text_field', wp_unslash( $_REQUEST[ self::$meta_background ] ) )
        );
     ...
     */
    public static function get_meta( $background = array() ) {
        ...
        if ( is_singular( self::$post_type ) ) {
            ...
            foreach ( self::$taxonomies as $tax ) {
                ...
                foreach ( get_the_terms( $object_id, $tax ) as $term ) {
                    $background = get_term_meta( $term->term_id, self::$term_meta, true );
