...
class Knife_User_Meta {
    ...
    public static function load_module() {
        ...
        add_filter( 'user_contactmethods', array( __CLASS__, 'add_user_contacts' ) );

        // Add user photo options
        add_action( 'admin_init', array( __CLASS__, 'add_user_options' ) );

        // Generate user photo
        add_action( 'wp_ajax_' . self::$ajax_action, array( __CLASS__, 'generate_user_photo' ) );

        // Update author description links with target attribute
        add_filter( 'get_the_author_description', array( __CLASS__, 'update_user_description' ) );
    }
    ...
    public static function add_user_options() {
        if ( current_user_can( 'edit_users' ) ) {
            ...
            add_action( 'show_user_profile', array( __CLASS__, 'print_user_options' ) );
            add_action( 'edit_user_profile', array( __CLASS__, 'print_user_options' ) );

            // Save user option
            add_action( 'profile_update', array( __CLASS__, 'save_user_meta' ) );

            // Enqueue user options scripts
            add_action( 'admin_enqueue_scripts', array( __CLASS__, 'enqueue_user_assets' ) );
        }
    }
    ...
    public static function add_user_contacts( $contact ) {
        $contact['vkontakte'] = esc_html__( 'Ссылка на ВКонтакте', 'knife-theme' );
        $contact['facebook']  = esc_html__( 'Ссылка на Facebook', 'knife-theme' );
        $contact['telegram']  = esc_html__( 'Профиль Telegram', 'knife-theme' );
        $contact['instagram'] = esc_html__( 'Профиль Instagram', 'knife-theme' );
        $contact['twitter']   = esc_html__( 'Профиль Twitter', 'knife-theme' );
     ...
     */
    public static function print_user_options( $user ) { // phpcs:ignore
    ...
    public static function save_user_meta( $user_id ) {
        if ( isset( $_REQUEST[ self::$user_photo ] ) ) {
            $photo = sanitize_text_field( wp_unslash( $_REQUEST[ self::$user_photo ] ) );
    ...
    public static function enqueue_user_assets( $hook ) {
        ...
        wp_enqueue_script( 'knife-user-options', $include . '/scripts/user-options.js', array( 'jquery' ), $version, true );

        $options = array(
            'user_id' => $user_id,
            'action'  => esc_attr( self::$ajax_action ),
            'nonce'   => wp_create_nonce( self::$ajax_nonce ),
            'choose'  => esc_html__( 'Выберите фото пользователя', 'knife-theme' ),
    ...
    public static function generate_user_photo() {
        ...
        if ( ! is_dir( $create['basedir'] ) && ! wp_mkdir_p( $create['basedir'] ) ) {
            wp_send_json_error( esc_html__( 'Проверьте права на запись папки загрузок', 'knife-theme' ) );
        }

        $filename = $user_id . uniqid( '-' ) . '.jpg';

        // Get image url by attachment id
        $poster = wp_get_image_editor( get_attached_file( absint( $_POST['poster'] ) ) );
