...
class Knife_Google_Search {
    ...
    public static function load_module() {
        ...
        add_action( 'parse_query', array( __CLASS__, 'disable_search' ), 9 );

        // Update query virtual page
        add_action( 'parse_query', array( __CLASS__, 'update_query_vars' ) );

        // Create custom search page rewrite url
        add_action( 'init', array( __CLASS__, 'add_search_rule' ) );

        // Add search query tag
        add_action( 'query_vars', array( __CLASS__, 'append_search_var' ) );

        // Include archive template for search results
        add_filter( 'template_include', array( __CLASS__, 'include_search' ) );
    ...
    public static function inject_object() {
        ...
        $options = array(
            'id'          => KNIFE_GCSE['id'],
            'placeholder' => esc_html__( 'Введите фразу для поиска', 'knife-theme' ),
        );
    ...
    public static function update_document_title( $title ) {
        if ( get_query_var( self::$query_var ) ) {
            $title['title'] = esc_html__( 'Поиск по материалам', 'knife-theme' );
        }
