...
class Knife_Invisible_Posts {
    ...
    public static function load_module() {
        ...
        add_action( 'init', array( __CLASS__, 'add_post_status' ) );

        // Display invisible state in posts list
        add_filter( 'display_post_states', array( __CLASS__, 'display_invisible_state' ), 10, 2 );
    }
    ...
    public static function add_post_status() {
        register_post_status(
            self::$post_status,
            array(
                'label'                     => esc_html__( 'Невидимый', 'knife-theme' ),
                'public'                    => true,
    ...
    public static function display_invisible_state( $states, $post ) {
        if ( get_post_status( $post->ID ) === self::$post_status ) {
            $states[] = esc_html__( 'Скрытый', 'knife-theme' );
        }
    ...
    public static function add_assets( $hook ) {
        ...
        // Insert admin script
        wp_enqueue_script( 'knife-invisible-status', $include . '/scripts/invisible-status.js', array( 'jquery' ), $version, true );

        $options = array(
            'current' => $post->post_status,
            'label'   => esc_html__( 'Скрытый', 'knife-theme' ),
