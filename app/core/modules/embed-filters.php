...
class Knife_Embed_Filters {
    ...
    public static function load_module() {
        ...
        // Wrap all embeds
        add_filter( 'embed_oembed_html', array( __CLASS__, 'append_markup' ), 12 );

        // Fix Yandex.Music iframe size
        add_filter( 'embed_oembed_html', array( __CLASS__, 'fix_yandex_embed' ), 10, 3 );

        // Instagram endpoint
        add_filter( 'oembed_providers', array( __CLASS__, 'add_instagram_endpoint' ) );

        // Facebook endpoints
        add_filter( 'oembed_providers', array( __CLASS__, 'add_facebook_endpoints' ) );

        // Twitter update
        add_filter( 'oembed_providers', array( __CLASS__, 'hide_twitter_thread' ) );

        // Add Yandex.Music to whitelisted provider
        add_filter( 'oembed_providers', array( __CLASS__, 'whitelist_yandex_music' ) );
     ...
     * @since 1.5
     */
    public static function append_markup( $html ) {
        if ( ! is_feed() ) {
    ...
    public static function whitelist_yandex_music( $providers ) {
        $providers['#https?://music.yandex.ru.*#i'] = array( '', true );
    ...
    public static function add_instagram_endpoint( $providers ) {
        ...
        // Add new endpoint
        $providers['#https?://(www\.)?instagr(\.am|am\.com)/(p|tv)/.*#i'] = array( $endpoint . '&hidecaption=true', true );
    ...
    public static function add_facebook_endpoints( $providers ) {
        ...
        // Add post endoints
        $providers['#https?://www\.facebook\.com/.*/posts/.*#i']       = array( $endpoint_post, true );
        $providers['#https?://www\.facebook\.com/.*/activity/.*#i']    = array( $endpoint_post, true );
        $providers['#https?://www\.facebook\.com/.*/photos/.*#i']      = array( $endpoint_post, true );
        $providers['#https?://www\.facebook\.com/photo(s/|\.php).*#i'] = array( $endpoint_post, true );
        $providers['#https?://www\.facebook\.com/permalink\.php.*#i']  = array( $endpoint_post, true );
        $providers['#https?://www\.facebook\.com/media/.*#i']          = array( $endpoint_post, true );
        $providers['#https?://www\.facebook\.com/questions/.*#i']      = array( $endpoint_post, true );
        $providers['#https?://www\.facebook\.com/notes/.*#i']          = array( $endpoint_post, true );

        // Add video endpoints
        $providers['#https?://www\.facebook\.com/.*/videos/.*#i']   = array( $endpoint_video, true );
        $providers['#https?://www\.facebook\.com/video\.php.*#i']   = array( $endpoint_video, true );
        $providers['#https?://www\.facebook\.com/watch/?\?v=\d+#i'] = array( $endpoint_video, true );
