...
class Knife_Snippet_Image {
    ...
    public static function load_module() {
        ...
        add_action( 'wp_ajax_' . self::$ajax_action, array( __CLASS__, 'generate_poster' ) );

        // Add snippet term fields
        add_action( 'admin_init', array( __CLASS__, 'add_term_fields' ) );
    }
    ...
    public static function add_term_fields() {
        foreach ( self::$taxonomies as $tax ) {
            add_action( "{$tax}_edit_form_fields", array( __CLASS__, 'print_term_row' ), 8, 2 );
            add_action( "edited_{$tax}", array( __CLASS__, 'save_term' ) );
        }
    }
     ...
     * @since 1.12
     */
    public static function print_term_row( $term, $taxonomy ) { // phpcs:ignore
    ...
    public static function add_metabox() {
        add_meta_box( 'knife-snippet-metabox', esc_html__( 'Изображение соцсети', 'knife-theme' ), array( __CLASS__, 'display_metabox' ), self::$post_type, 'side' );
    }
    ...
    public static function save_term( $term_id ) {
        ...
        $snippet = sanitize_text_field( wp_unslash( $_REQUEST[ self::$term_meta ] ) );
    ...
    public static function enqueue_assets( $hook ) {
        ...
        // Insert admin styles
        wp_enqueue_style( 'knife-snippet-metabox', $include . '/styles/snippet-metabox.css', array(), $version );

        // Insert admin scripts
        wp_enqueue_script( 'knife-snippet-metabox', $include . '/scripts/snippet-metabox.js', array( 'jquery' ), $version, true );
    ...
    public static function save_meta( $post_id ) {
        ...
        $image = sanitize_text_field( wp_unslash( $_REQUEST[ self::$meta_image ] ) );
