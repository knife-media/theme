...
class Knife_Best_Posts {
    ...
    public static function load_module() {
        ...
        add_action( 'init', array( __CLASS__, 'add_best_rule' ) );

        // Update query_vars for best archive
        add_action( 'parse_query', array( __CLASS__, 'update_query_vars' ) );

        // Add share query tag
        add_action( 'query_vars', array( __CLASS__, 'append_best_var' ) );
    ...
    public static function update_archive_title( $title ) {
        if ( get_query_var( self::$query_var ) ) {
            $title = sprintf(
                ...
                esc_html__( 'Выбор редакции', 'knife-theme' )
            );
        }
    ...
    public static function update_document_title( $title ) {
        if ( get_query_var( self::$query_var ) ) {
            $title['title'] = esc_html__( 'Выбор редакции', 'knife-theme' );
        }
